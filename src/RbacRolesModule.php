...
final class RbacRolesModule implements ModuleInterface
{
    public function name(): string { return 'table-rbac_roles'; }
    public function table(): string { return 'rbac_roles'; }
    ...
    public static function contractView(): string { return 'vw_rbac_roles'; }
...
SQL;
    ...
    public function status(Database $db, SqlDialect $d): array
    {
        ...
        $expectedIdx = [ 'idx_rbac_roles_repo', 'idx_rbac_roles_status' ];
        if ($d->isMysql()) {
    ...
    public function info(): array
    {
        return [
            ...
            'indexes'     => [ 'idx_rbac_roles_repo', 'idx_rbac_roles_status' ],
            'foreignKeys' => [ 'fk_rbac_roles_repo' ],
        ];
    }
}
