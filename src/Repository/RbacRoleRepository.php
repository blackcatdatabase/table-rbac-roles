...
use BlackCat\Database\Packages\RbacRoles\Repository\RbacRoleRepositoryInterface;
...
class RbacRoleRepository implements RbacRoleRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'repo_id', 'slug' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\RbacRoles\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'name', 'description', 'version', 'status', 'updated_at' ]);
          return count($rows);
        }
    ...
    public function getByRepoIdAndSlug(int $repoId, string $slug, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRoles\Dto\RbacRoleDto|null {
        return $this->getByUnique([ 'repo_id' => $repoId, 'slug' => $slug ], $asDto);
    }
    ...
    public function getIdByRepoIdAndSlug(int $repoId, string $slug) {
        $row = $this->getByRepoIdAndSlug($repoId, $slug, false);
