...
namespace BlackCat\Database\Packages\RbacRoles\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RbacRoles\Definitions;
use BlackCat\Database\Packages\RbacRoles\Criteria;
use BlackCat\Database\Packages\RbacRoles\Dto\RbacRoleDto as Dto;
use BlackCat\Database\Packages\RbacRoles\Mapper\RbacRoleDtoMapper as RowMapper;
...
final class RbacRoleRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RbacRoles\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'repo_id', 'slug' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RbacRoles\Definitions::table());
            return $bulk->upsertMany($rows, [ 'repo_id', 'slug' ], [ 'name', 'description', 'version', 'status', 'updated_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacRoles\Dto\RbacRoleDto|null */
    public function getByRepoIdAndSlug(int $repoId, string $slug, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRoles\Dto\RbacRoleDto|null {
        $row = $this->getByUnique([ 'repo_id' => $repoId, 'slug' => $slug ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RbacRoles\Mapper\RbacRoleDtoMapper::fromRow($row);
    }
    public function existsByRepoIdAndSlug(int $repoId, string $slug): bool {
        $where = 't.' . Ident::q($this->db, 'repo_id') . ' = :uniq_repo_id' . ' AND ' . 't.' . Ident::q($this->db, 'slug') . ' = :uniq_slug';
        return $this->exists($where, [ 'uniq_repo_id' => $repoId, 'uniq_slug' => $slug ]);
    }
    /** @return int|string|null */
    public function getIdByRepoIdAndSlug(int $repoId, string $slug) {
        $row = $this->getByRepoIdAndSlug($repoId, $slug);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacRoles\Dto\RbacRoleDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRoles\Dto\RbacRoleDto|null {
