...
namespace BlackCat\Database\Packages\RbacRoles\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RbacRoles\Dto\RbacRoleDto;
use BlackCat\Database\Packages\RbacRoles\Mapper\RbacRoleDtoMapper;
 ...
 */
final class RbacRolesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RbacRoleRepository $rbacRoleRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rbac_roles' => $this->rbacRoleRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RbacRoles\ModuleException("Record {$idStr} not found for locking.");
            }
