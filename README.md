...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 1 outbound / 2 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 1 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\rbac-roles](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\rbac-roles)
...
graph LR
  ...
  rbac_roles["rbac_roles"]:::center
  rbac_roles -->|FK| rbac_repositories["rbac_repositories"]:::link
  rbac_role_permissions["rbac_role_permissions"]:::inbound -->|FK| rbac_roles
  rbac_user_roles["rbac_user_roles"]:::inbound -->|FK| rbac_roles
...
- Inbound (relies on this): "rbac_role_permissions", "rbac_user_roles"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package rbac-roles -Apply
